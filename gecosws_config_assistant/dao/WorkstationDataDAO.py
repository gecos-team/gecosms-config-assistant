...
class WorkstationDataDAO(object):
    ...
    def __init__(self):
		...
		if sys.platform == 'win32':
			self.pclabel_file = os.path.join('C:\\','etc', 'pclabel')
			self.gcc_control_file = os.path.join('C:\\','etc', 'gcc.control')
		else:
    ...
    def load(self):
        ...
        if json_data is not None:
            data.set_node_name(json_data['gcc_nodename'].encode('utf-8'))  
    ...
    def save(self, data):
        ...
        template = Template()
        template.source = get_data_file(os.path.join('templates','gcc.control'))
